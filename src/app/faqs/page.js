      ...
      <div
        ...
        <div className="w-[75%] h-[75vh] bg-white rounded-xl shadow-xl flex flex-col p-6 border border-gray-200">
          ...
          {showShortcuts && (
            <div className="flex flex-wrap justify-center gap-2 mb-4">
              {faqData.map(({question}, index) => (
                <button key={index}
                        className="px-6 py-3 rounded-full bg-white border border-gray-300 hover:bg-[#057e7e] hover:text-white font-semibold shadow-md"
          ...
          <form onSubmit={(e) => {
            e.preventDefault();
            sendMessage(input);
          }} className="flex items-center border-t border-gray-300 pt-3 px-3">
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
                   placeholder="Type your question..." className="flex-1 p-3 rounded-full border border-gray-300"
                   disabled={loading}/>
            <button type="submit" disabled={loading || input.trim() === ""}
                    className="ml-3 bg-[#057e7e] text-white p-3 rounded-full shadow-md">{loading ?
              <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-white"></div> :
              <FaPaperPlane className="w-5 h-5"/>}</button>
          </form>
        </div>
      </div>
    </>
